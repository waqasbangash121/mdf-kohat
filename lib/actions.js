...
export async function updateCattle(id, data) {
  try {
    ...
    // Handle marketPrice updates
    if (data.marketPrice !== undefined) {
      if (data.marketPrice && data.marketPrice !== '' && data.marketPrice !== null) {
        updateData.marketPrice = parseInt(data.marketPrice);
      } else {
        updateData.marketPrice = null; // Allow clearing the market price
      }
    }
...
export async function deleteCattle(id) {
  try {
    ...
    if (transactionCount > 0) {
      // If there are transactions, mark as sold instead of actual deletion
      ...
      revalidatePath('/cattle');
      revalidatePath('/cashflow');
      return { success: true, message: 'Cattle marked as sold due to existing transaction history' };
    } else {
      ...
      revalidatePath('/cattle');
      revalidatePath('/cashflow');
      return { success: true, message: 'Cattle deleted successfully' };
    }
  } catch (error) {
    ...
    if (error.code === 'P2003') {
      ...
      throw new Error('Cannot delete cattle: has associated records');
    } else if (error.code === 'P2025') {
      // Record not found
      throw new Error('Cattle not found');
    } else {
      throw new Error(`Failed to delete cattle: ${error.message}`);
    }
  }
}
...
export async function updateTransaction(id, data) {
  try {
    ...
    if (data.category === 'milk_sales') {
      ...
      // Handle cattle purchase details
      if (data.category === 'cattle_purchase' && data.cattleName) {
        details.cattleDetails = {
          name: data.cattleName,
          type: data.cattleType,
